  ...
  getAdapter: (adapters, config) => {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (!isResolvedHandle(nameOrAdapter)) {
        adapter3 = knownAdapters[(id2 = String(nameOrAdapter)).toLowerCase()];
        if (adapter3 === void 0) {
      ...
      rejectedReasons[id2 || "#" + i] = adapter3;
    }
    if (!adapter3) {
...
// node_modules/.pnpm/youtube-transcript-api@3.0.6/node_modules/youtube-transcript-api/src/index.js
function generateRandomHex(size) {
  return [...Array(size)].map(() => Math.floor(Math.random() * 16).toString(16)).join("");
}
var TranscriptClient = class {
  ready;
  // ready event trigger
  #instance;
  // Axios Instance
  ...
  #get_firebase_cfg_creds() {
    return (async () => {
      const { data: data2 } = await this.#instance.get("/");
      const $2 = load(data2);
      for (const elem of $2("script[src]").toArray()) {
        const url = $2(elem).attr("src");
  ...
  #get_auth() {
    ...
    return (async () => {
      const { data: data2 } = await this.#instance.post(url, {
   ...
   * @returns Firebase auth details
   */
  #get_x_client_context(id2) {
    return (async () => {
      const { data: data2 } = await this.#instance.get("/videos/" + id2);
      const $2 = load(data2);
      for (const elem of $2("script[src]").toArray()) {
        const url = $2(elem).attr("src");
        const { data: script } = await this.#instance.get(url);
   ...
   */
  async getTranscript(id2, config) {
    const auth = await this.#get_auth();
    const x_header = await this.#get_x_client_context(id2);
    try {
      const { data: data2 } = await this.#instance.post("/api/transcripts", {
        ids: [id2]
      }, {
        ...config || {},
        headers: {
...
export {
  TranscriptClient as default
};
//# sourceMappingURL=youtube-transcript-api.js.map

...
import { useLayoutStore, type RightPanelTab } from "@/hooks/useLayoutStore";
...
const TAB_ICONS: Record<RightPanelTab, React.ReactNode> = {
  herramientas: <Wand2 className="w-4 h-4" />,
  fuentes: <FilePlus className="w-4 h-4" />,
  ajustes: <Settings className="w-4 h-4" />,
};

export default function NotebookRightPanelTabs({
  notebook,
  onUpdateNotebook,
}: Props) {
  const {
  ...
  const selectAll = () =>
    setSelectedSourceIds(allSources.slice(0, MAX_SOURCES).map(s => s.id));
  const deselectAll = () => setSelectedSourceIds([]);

  const getSelectedSources = (): SourceItem[] => {
    if (selectedSourceIds.length === 0) return allSources.slice(0, MAX_SOURCES);
  ...
  const handleGenerateWithContext = (context: string) => {
    ...
    if (type === "summary") handleGenerateSummaryInternal(context);
    else if (type === "flashcards") handleGenerateFlashcardsInternal(context);
    else if (type === "test") handleGenerateTestInternal(context);
    else if (type === "mindmap") handleGenerateDiagramInternal(context);
  };
  ...
  const pollPodcastStatus = async () => {
    ...
    try {
      ...
      if (data.script) setCurrentPodcastScript(data.script);
  ...
  const GENERATION_TOOLS = [
    ...
    },
    {
      ...
      handler: () => {
        if (allSources.length > 0) setShowPodcastConfig(true);
      },
    },
    {
      id: "test",
  ...
  return (
    <div
      className={cn(
        ...
        rightPanelTab === "fuentes" ? "w-80" : "w-96"
      )}
    >
      {/* Header with toggle */}
      <div className="p-4 border-b border-border flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button
            ...
            title={leftPanelOpen ? "Ocultar panel" : "Mostrar panel"}
          >
            {leftPanelOpen ? (
      ...
      >
        <TabsList className="mx-4 mt-2 justify-start gap-1 h-9">
          <TabsTrigger value="herramientas" className="text-xs gap-1.5 px-3">
            <Wand2 className="w-3.5 h-3.5" />
            <span>Herramientas</span>
          </TabsTrigger>
          <TabsTrigger value="fuentes" className="text-xs gap-1.5 px-3">
            <FilePlus className="w-3.5 h-3.5" />
            <span>Fuentes</span>
          </TabsTrigger>
          <TabsTrigger value="ajustes" className="text-xs gap-1.5 px-3">
        ...
        >
          {GENERATION_TOOLS.map(tool => (
            <button
              ...
              className={cn(
                "w-full p-4 rounded-lg transition-all text-left disabled:opacity-50",
                generating === tool.id
            ...
            >
              <div className="flex items-start gap-3">
                ...
                >
                  ...
                  >
                    {generating === tool.id ? "Generando..." : tool.description}
                  </p>
                </div>
              </div>
            </button>
        ...
        >
          ...
          <button
            ...
            className="w-full p-2 bg-secondary/50 hover:bg-secondary rounded-lg transition-colors flex items-center justify-center gap-2 text-sm text-muted-foreground hover:text-foreground"
          >
          ...
          <div>
            ...
            <label className="flex items-center gap-3 p-3 bg-secondary/50 rounded-lg cursor-pointer">
              <input
                ...
                onChange={e => setLeftPanelOpen(e.target.checked)}
                className="w-4 h-4"
              />
              <span className="text-sm">Panel izquierdo visible</span>
            </label>
          </div>
          <div>
            <h3 className="text-sm font-semibold mb-3">Acerca de StudyOS</h3>
            <p className="text-xs text-muted-foreground">
              StudyOS v3 - Sistema de estudio con IA
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              Powered by Claude + Groq + TTS
            </p>
          </div>
        </TabsContent>
      </Tabs>
